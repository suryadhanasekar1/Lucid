...
export async function sendAtlasMessage(args: SendArgs): Promise<SendResult> {
  const body: AtlasChatRequest = {
  ...
  while (true) {
    ...
    while ((idx = buffer.indexOf("\n\n")) !== -1) {
      ...
      if (!line) continue;
      let frame: AtlasStreamFrame;
      try {
        frame = JSON.parse(line) as AtlasStreamFrame;
      } catch {
        continue;
      }
      if (frame.type === "meta") {
